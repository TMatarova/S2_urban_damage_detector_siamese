...
class SiameseCNN(nn.Module):
    def __init__(self, in_channels=3):
        super().__init__()
        # 1) Load pretrained ResNet-34 for the backbone
        # Use a feature extractor for each input image
        ...
        # 3) Keep features up to the global average pooling layer which makes the output 512 dimensional vector
        self.features = nn.Sequential(*list(resnet.children())[:-1])  # -> (N, 512, 1, 1)

        # 4) MLP head
        # Each image goes through ResNet -> 512-dim feature vector
        # For a pair of embeddings we combine them in 4 ways: [before, after, difference and product]
        # that makes 512 * 4 = 2048 features total
        # The head is a small feed-forward network that takes this 2048-dim vector which goes through layers to final logit
        self.head = nn.Sequential(
            nn.Linear(2048, 1024), nn.BatchNorm1d(1024), nn.GELU(), nn.Dropout(0.5),
    ...
    def forward(self, before, after):
        # Extract 512-dim embeddings for both inputs of the heads
        feature_before, feature_after = self.forward_once(before), self.forward_once(after)
        combined_features = torch.cat([feature_before, feature_after, torch.abs(feature_before - feature_after), feature_before * feature_after], dim=1) #(N, 2048)
